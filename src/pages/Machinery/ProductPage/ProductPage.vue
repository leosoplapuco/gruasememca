...
const videoUrl = computed(() => {
  // Verificar si existe el array video y si tiene al menos un elemento con url
  if (
    machine.value?.video &&
    Array.isArray(machine.value.video) &&
    machine.value.video.length > 0
  ) {
    const firstVideo = machine.value.video[0]
    if (firstVideo.url && firstVideo.url.trim() !== '') {
    ...
    <div class="block-content">
      <section class="machinery-page-container">
        <div class="machinery-page-tag machinery-page-tag-1">
          ...
          <!-- ProductVideo solo se renderiza si hay URL de video -->
          <ProductVideo v-if="videoUrl" :video-url="videoUrl" />
